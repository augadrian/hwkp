...
import com.hwkp.common.pojo.AjaxResult;
import com.hwkp.common.pojo.PageAjax;
import com.hwkp.common.pojo.ParamData;
import com.hwkp.common.utils.AppUtil;
import com.hwkp.mapper.DaoSupport;
...
public abstract class BaseService {
    ...
    public AjaxResult findForObject(String str, String[] args) {
    	...
    	if(StringUtils.isEmpty(result)){
			data = daoSupport.findForObject(str, params);
    	}
    ...
    public AjaxResult findForList(String str, String[] args) {
    	...
    	List<?> list = null;
    	if(StringUtils.isEmpty(result)){
    		list = daoSupport.findForList(str, params);
    	}
	...
	public <T> PageAjax<T> findPage(String str, String[] args){
    	...
    	if(StringUtils.isEmpty(result)){
    		AppUtil.startPage(params);
    		list = (List<T>) daoSupport.findForList(str, params);
    	}
    ...
    public Map<?, ?> findForMap(String str, String key) {
    	return daoSupport.findForMap(str, new ParamData(), key);
    }
    ...
    public AjaxResult save(String str, String[] args) {
    	...
    	String result = AppUtil.checkParam(params, args);
    	if(StringUtils.isEmpty(result)){
    		int ret = daoSupport.save(str, params);
    		if(ret < 1){
    			result = "添加失败";
    		}
    	}
    ...
    public AjaxResult batchSave(String str, List<?> list) {
    	...
    	if(CollectionUtils.isNotEmpty(list)){
        	int ret = daoSupport.batchSave(str, list);
        	if(ret > 0){
        		result = null;
        	}
    	}
    ...
    public AjaxResult update(String str, String[] args) {
    	...
    	String result = AppUtil.checkParam(params, args);
    	if(StringUtils.isEmpty(result)){
    		int ret = daoSupport.update(str, params);
    		if(ret < 1){
    			result = "修改失败";
    		}
    	}
    ...
    public AjaxResult batchUpdate(String str, List<?> list) {
    	String result = "修改失败";
    	if(CollectionUtils.isNotEmpty(list)){
        	int ret = daoSupport.batchUpdate(str, list);
        	if(ret > 0){
        		result = null;
        	}
    	}
    ...
    public AjaxResult delete(String str, String[] args) {
    	...
    	String result = AppUtil.checkParam(params, args);
    	if(StringUtils.isEmpty(result)){
    		int ret = daoSupport.delete(str, params);
    		if(ret < 1){
    			result = "删除失败";
    		}
    	}
    ...
    public AjaxResult batchDelete(String str, Integer[] ids) {
    	...
    	if(null != ids && ids.length > 0){
        	...
        	int ret = daoSupport.batchDelete(str, list);
        	if(ret > 0){
        		result = null;
        	}
    	}
